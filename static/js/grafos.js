...
// VISUALIZACION PRINCIPAL POR NUMERO DE SEGUIDORES
...
function pintarGrafo(tipoGrafo, nodos) {
    ...
    var node = svg.selectAll(".node")
        ...
        .attr("class", "node")
        // Tamaño en funcion del numero de seguidores
        .attr("r", function (d) {
                    if (d['num_followers'] > 10000000) {
                        return 175 ;
                    }
                    else if (d['num_followers'] >= 5000000) {
                        // return 4 + (0.001 * d['num_followers'] / 100);
                        return 150;
                    }
                    else if (d['num_followers'] >= 1000000) {
                        // return 4 + (0.005 * d['num_followers'] / 100);
                        return 100;
                    }
                    else if (d['num_followers'] >= 500000) {
                        // return 4 + (0.01 * d['num_followers'] / 100);
                        return 75;
                    }
                    else if (d['num_followers'] >= 100000) {
                        // return 4 + (0.03* d['num_followers'] / 100);
                        return 50;
                    }
                    else if (d['num_followers'] >= 50000) {
                        // return 4 + (0.03 * d['num_followers'] / 100);
                        return 25;
                    }
                    else if (d['num_followers'] >= 10000) {
                        return 4+ (0.1 * d['num_followers'] / 100);
                    }
                    else {
                        return 4 + (0.1 * d['num_followers'] / 100);
                    }
